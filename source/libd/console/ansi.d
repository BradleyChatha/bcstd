...
struct AnsiColour
{
    ...
    /++
    + Converts this `AnsiColour` into a sequence String.
    +
    + See_Also:
    +  `toSequence`
    + ++/
    ...
    {
        char[MAX_CHARS_NEEDED] chars;
        return String(this.toSequence(chars[0..MAX_CHARS_NEEDED]));
    }
    ///
    @("AnsiColour.toString")
    unittest
...
struct AnsiStyle
{
    ...
    /++
    + Converts this `AnsiStyle` into a sequence string.
    +
    + See_Also:
    +  `toSequence`
    + ++/
...
struct AnsiTextLite
{
    ...
    /++
     ...
     +
     + Returns: 
     +  A string containing this `AnsiTextLite` as an ANSI-encoded string, ready for printing.
     + ++/
    ...
    String toString() const
    {
        ...
        String buffer;
        buffer.length = styleSlice.length + this.text.length + ANSI_COLOUR_RESET.length;
...
struct AnsiText(alias ImplementationMixin)
{
    ...
    // Generate a String-based toString if circumstances allow.
    static if(
    ...
    )
    {
        /++
         ...
         + implementation doesn't define its own `toString then
         ...
         +
         +  For $(B this default generated) implementation of `toString`, it is a seperate String so is
...
unittest
{
    import libd.data.conv : to;

    void test(AnsiStyleSet sourceAndExpected)
    {
        char[AnsiStyleSet.MAX_CHARS_NEEDED] buffer;
        String sequence;
        sequence.putMany(ANSI_CSI, sourceAndExpected.toSequence(buffer), ANSI_COLOUR_END);

        AnsiStyleSet got;
        size_t charsRead;
        const error = ansiExecuteSgrSequence(sequence[0..$], got, charsRead);
        if(error !is null)
            assert(false, error);

        assert(charsRead == sequence.length);
        assert(sourceAndExpected == got);
    }
